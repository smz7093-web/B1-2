# 2차 미션 학습지 — 멀티모달 콘텐츠 제작

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# 문서 1 · 스토리보드(기획) 문서 (PDF 1개)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

## STEP 1 · 브랜드 1개 정하기

예당 스토리 테이블(로컬 관광 콘텐츠)

의미: 예당호의 이야기가 담긴 식탁

강점: 걷기-듣기-맛보기의 전체 여정을 '식탁'이라는 중심 이미지로 통합

타깃: 로컬 미식 여행객, 스토리텔링을 중시하는 30-60대

## STEP 2 · 브랜드 아이덴티티 정의하기

예당 스토리 테이블 - 브랜드 아이덴티티 정의서

**1) 브랜드명:
예당 스토리 테이블

예당호의 풍경, 역사, 사람, 식재료가 모두 모이는 '식탁'
단순한 식사가 아닌 '이야기가 담긴 경험'을 강조

**2) 타겟 고객층
주요 타깃
연령: 30대~60대 (경제력 있는 성인)
특성:
로컬 미식과 스토리텔링에 관심 있는 여행객
단순 관광이 아닌 '의미 있는 경험'을 추구하는 사람들
지역 문화와 농업 가치를 이해하고 공감하는 고객
가족, 소규모 모임, 특별한 날을 기념하는 그룹
부가 타깃:
로컬푸드, 지속가능한 여행에 관심 있는 MZ세대 (30대 초반)
농촌 체험과 교육적 경험을 원하는 가족 단위

**3) 톤앤매너 (감성 키워드)
핵심 감성

감성적 - 예당호의 역사와 수몰마을의 기억을 담은 따뜻함
고급스러움 - 프리미엄 소규모 운영, 정성 있는 서비스
진정성 - 농부의 목소리가 살아있는 진솔한 경험
느린 여행 - 걷고, 듣고, 맛보는 차분하고 명상적인 리듬
로컬 중심 - 지역 고유의 이야기와 자산을 존중하는 태도
표현 방식 -
세련되되 거리감 없는 (친근한 고급스러움)
교육적이되 편안한 (자연스러운 스토리텔링)
프리미엄이되 자연친화적인 (소박한 우아함)

**4) 차별점 (USP)
경쟁 브랜드와의 핵심 강점

구분		일반 관광지	일반 농촌 체험	예당 스토리 테이블
경험 구조		사진 촬영 중심	식사 제공 중심	걷기-듣기-맛보기의 완결된 여정
스토리		없음		단편적		예당호 형성사, 수몰마을, 농부 이야기의 유기적 연결
생산자 참여	없음		간접적		농부가 직접 참여해 식재료 스토리 전달
운영 규모		대규모		다양함		10-16명 소규모 프리미엄 (품질 관리 강화)
지역 일자리	제한적		제한적		50+ 시니어 해설사, 진행인력으로 상생 구조 구축
체류 효과		2시간 이하	2-3시간		4-5시간 반나절 체험 (추가 소비 유도)

핵심 차별점 3가지
내륙형 스토리 파인다이닝 - 해양 중심 모델이 아닌 수변 호수의 맥락에 맞춘 재해석
농부의 주인공화 - 단순 식재료 공급자가 아닌 스토리텔러로서의 역할
완결된 여정 설계 - 이동-해설-식사가 하나의 서사 구조로 통합된 경험

**5) 브랜드가 전달하고 싶은 이미지
핵심 메시지

"예당호를 따라 걷고, 지역의 기억을 듣고, 농부의 손으로 만든 밥상을 나눈다"

구체적 이미지

🌾 지역성: 예산의 역사, 농업, 사람이 살아있는 경험

🚶 여정성: 도착이 아닌 과정 자체가 의미 있는 여행

🍽️ 공유성: 농부, 해설사, 방문객이 함께 만드는 식탁

⏰ 느림의 가치: 빠른 관광이 아닌 4시간의 깊이 있는 몰입

🌱 지속가능성: 제철 로컬푸드, 지역 상생, 환경 존중

방문객이 느껴야 할 경험
"단순한 관광이 아니라 예산이라는 지역을 이해하는 경험을 했다"
"농부의 얼굴과 이야기가 담긴 음식을 먹었다"
"예당호의 풍경이 내 몸과 마음에 남아있다"
"다시 찾고 싶은 특별한 밥상이 있다"

📌 브랜드 아이덴티티 요약
| 구분 | 내용 |
| :--- | :--- |
| **브랜드명** | **예당 스토리 테이블** |
| **슬로건** | 예당호를 걷고, 지역의 이야기를 듣고, 농부의 밥상을 맛본다 |
| **타겟** | 30-60대 로컬 미식 여행객 |
| **톤앤매너** | 감성적, 고급스러움, 진정성, 느린 여행, 로컬 중심 |
| **USP (차별점)** | 내륙형 스토리 파인다이닝 + 농부 주인공화 + 완결된 여정 설계 |
| **브랜드 이미지** | 예당호의 역사와 사람, 농부의 손이 담긴 프리미엄 로컬 미식 여행 |

## STEP 3 · 광고 목적 정하기
신제품 소개

## STEP 4 · 핵심 메시지 1문장 만들기
예당 스토리 테이블 - 광고 핵심 메시지 정교화

### 📌 핵심 메시지 : "예당호를 걷고, 지역의 이야기를 듣고, 농부의 밥상을 맛본다"

| 구분 | 내용 |
| :--- | :--- |
| **타깃 (Who)** | 예당호를 찾는 모든 방문객 |
| **핵심 가치 (Value)** | 단순 관광이 아닌 '지역 이해' + '생산자와의 만남' |
| **감성 톤 (Mood)** | 차분하고 의미 있는 여정 (느린 여행의 리듬) |
| **청각 연출 (Audio)** | 발걸음 소리 → 해설사의 목소리 → 농부와 대화 → 함께하는 밥상 |
| **시각 연출 (Visual)** | 호수 풍경 → 마을 이야기 → 식탁 위의 음식 |

---

🎬 10초 광고 시나리오 예시

### 🎬 메시지 1: 예당호 여정 중심

| 시간 | 연출 장면 (Visual & Audio) |
| :---: | :--- |
| **0 ~ 3초** | 마을해설사의 목소리 *"이 다리는 오랜 역사를 품고 있죠"* **자막:** *"예당호 출렁다리"*|
| **4 ~ 7초** | 내래이션 *"예당호의 신선함이 그대로 담겨 있습니다."* 농부의 얼굴, 식탁 위의 음식 **자막:** *"예당의 맛"*|
| **7 ~ 10초** | 내래이션 *"맛있게 드시고 좋은 추억 만드세요."* **자막:** *"예당호 스토리 테이블 온기로"*<br>**브랜드:** 예당 스토리 테이블 온기로 |

---

## STEP 5 · 영상 전체 방향 설계하기

📋 최종 확정 내용

| 구분 | 내용 |
| :--- | :--- |
| **브랜드명** | **예당 스토리 테이블** |
| **슬로건** | 예당호를 걷고, 지역의 이야기를 듣고, 농부의 밥상을 맛본다 |
| **타겟** | 30-60대 로컬 미식 여행객 |
| **톤앤매너** | 감성적, 고급스러움, 진정성, 느린 여행, 로컬 중심 |
| **USP (차별점)** | 내륙형 스토리 파인다이닝 + 농부 주인공화 + 완결된 여정 설계 |
| **브랜드 이미지** | 예당호의 역사와 사람, 농부의 손이 담긴 프리미엄 로컬 미식 여행 |

🎯 이 조합이 강력한 이유
✨ 슬로건의 3단계 구조

걷고 → 예당호의 풍경과 공간 경험
듣고 → 지역의 역사와 이야기 (수몰마을, 농부의 목소리)
맛본다 → 농부의 손으로 만든 음식 (최종 경험)
✨ 브랜드명과의 완벽한 시너지

"스토리 테이블" = 이야기가 담긴 식탁
슬로건이 그 이야기의 과정을 구체적으로 설명
✨ 차별점이 모두 담김

걷기 → 소규모 프리미엄 운영 (4-5시간 여정)
듣기 → 스토리텔링 (수몰마을, 지역 기억)
맛보기 → 농부 참여 (생산자와의 만남)

## STEP 6 · 씬 수정하기
예당 스토리 테이블 - 10초 광고 영상 씬 구성
🎬 추천 씬 구성: 3씬 구조
10초를 3개 씬으로 나누면 씬당 3~4초 → 명확하고 임팩트 있음

| 시간 | 구분 | 내용 | 비고 |
| :---: | :---: | :---: | :---: |
| **0 ~ 3초** | 씬 1 | **오프닝, 경험 과정** | 마을해설사의 목소리 |
| **4 ~ 7초** | 씬 2 | **농부의 얼굴, 음식** | 걷고, 듣고 |
| **7 ~ 10초** | 씬 3 | **핵심 가치, 마무리** | 브랜드 노출 |

📍 각 씬 상세 구성

씬 1: 오프닝 - 예당호 풍경 (0-3초)
영상 내용

마을해설사가 방문객들에게 이야기하는 장면
또는 농부와 대화하는 장면
지역의 역사/이야기가 담긴 순간 포착
목적: 슬로건의 "걷고, 듣고" 부분 시각화

자막: "예당호 출렁다리" (자막 나타남)

음향: 해설사의 목소리 또는 대화음

씬 2: 핵심 가치 - 농부의 밥상 (4-7초)
영상 내용

식탁에 차리는 농부와 한상 차려진 음식을 클로즈업
따뜻한 식탁의 순간
목적: 차별점인 "농부의 밥상" 강조 + 감정 전달

자막: "예당의 맛" (자막 나타남)

음향: 내래이션 "예당호의 신선함이 그대로 담겨 있습니다."

씬 3: 브랜드 노출 + 마무리 (7-10초)
영상 내용

브랜드명 + 슬로건 전체 노출
로고 + 웹사이트/예약 정보
또는 예당호 풍경으로 돌아오는 마무리 컷
목적: 브랜드 인식 + 행동 유도 (예약, 문의)
자막: 예당 스토리 테이블
"예당호 스토리 테이블 온기로"

음향: 잔잔한 배경음악 + 브랜드 사운드 로고 (선택)
      내래이션 "맛있게 드시고 좋은 추억 만드세요"

🎞️ 타임라인 시각화
| 씬 1 | 씬 2 | 씬 3 |
| :---: | :---: | :---: |
| **걷고, 듣고**<br>*(경험)* | **밥상 맛보기**<br>*(가치)* | **브랜드 노출**<br>*(마무리)* |

## STEP 7 · 스토리보드 문서의 씬별 필수 항목 채우기

# 예당 스토리 테이블 - 10초 광고 영상 스토리보드
---
## 📋 스토리보드 문서 템플릿

## 🎬 씬 1: 오프닝, 경험 과정 - 걷고, 듣고

### 기본 정보
| 항목 | 내용 |
|------|------|
| **씬 번호** | 1 |
| **씬 길이** | 3초 |
| **목표 메시지** | 예당호를 걸으며 지역의 이야기를 듣는 경험을 시각화한다 |

### 화면 구성
| 항목 | 내용 |
|------|------|
| **구도** | 클로즈업 + 와이드샷 조합 (16:9) |
| **피사체** | 해설사(또는 농부)가 방문객들에게 이야기하는 모습, 방문객들의 표정 |
| **배경** | 예당호 수변길, 마을 풍경, 역사적 건물 또는 수몰마을 흔적 |
| **텍스트 유무** | 있음 - 관광지 명 자막처리 |

### 내레이션 및 음향
| 항목 | 내용 |
|------|------|
| **내레이션** | 해설사의 목소리 일부 사용 |
| **화면 카피** | "예당호 출렁다리" (자막, 1.5초 노출) |
| **배경음악/음향** | 해설사의 목소리 + 잔잔한 배경음악 (음량 낮게) |

### 제작 도구 및 방법

#### 📸 이미지 생성 도구
| 항목 | 내용 |
|------|------|
| **도구명** | Gemini Flash |
| **사용 목적** | 해설사와 방문객이 상호작용하는 장면 생성 (실제 촬영이 어려운 경우) |
| **입력 프롬프트** | `A local storyteller or farmer explaining the history of Yedang Lake to visitors, warm and engaging atmosphere, natural lighting, cinematic composition, 16:9` |
| **출력 결과 요약** | 지역 해설사가 방문객들에게 이야기하는 따뜻한 장면 |
| **생성 파일명** | `Scene1_Storyteller_Interaction_v1.png` |

#### 🎥 비디오 생성/변환 도구
| 항목 | 내용 |
|------|------|
| **도구명** | Flow |
| **사용 목적** | 여러 각도의 영상 편집, 자막 추가, 색감 보정 |
| **입력 프롬프트** | `Edit 3-second sequence: close-up of storyteller's face → wide shot of group listening → close-up of visitor's reaction, smooth transitions, warm color grading` |
| **출력 결과 요약** | 3초의 상호작용 장면 영상 |
| **생성 파일명** | `Scene1_Storytelling_Video_3s_v1.mp4` |

#### 🎵 오디오 생성 도구
| 항목 | 내용 |
|------|------|
| **도구명** | ElevenLabs (AI 음성) |
| **사용 목적** | 해설사의 목소리 |
| **입력 프롬프트** | `Generate a warm, engaging voice narrating local history of Yedang Lake, 3 seconds, Korean language, natural tone` |
| **출력 결과 요약** | 지역 해설사의 따뜻한 목소리 오디오 |
| **생성 파일명** | `Scene1_Audio_Storyteller_Voice_v1.mp3` |

#### 📝 자막 생성 도구
| 항목 | 내용 |
|------|------|
| **도구명** | CapCut |
| **사용 목적** | 슬로건의 일부 자막 추가 |
| **입력 텍스트** | "예당호 출렁다리" |
| **출력 결과 요약** | 화면 하단에 나타나는 자막 (1.5초 노출) |
| **생성 파일명** | `Scene1_Subtitle_v1.srt` |

### 최종 산출물
| 항목 | 내용 |
|------|------|
| **최종 영상 파일** | `Scene1_Complete_3s.mp4` |
| **해상도** | 1920x1080 (Full HD) |
| **프레임레이트** | 30fps |
| **비트레이트** | 5000kbps |

---

## 🎬 씬 2: 핵심 가치 - 요리연구가의 스토리 다이닝

### 기본 정보
| 항목 | 내용 |
|------|------|
| **씬 번호** | 2 |
| **씬 길이** | 3초 |
| **목표 메시지** | 세련된 요리연구가의 미식 퍼포먼스로 예당호 식재료의 프리미엄한 가치를 전달한다 |

### 화면 구성
| 항목 | 내용 |
|------|------|
| **구도** | 미디엄/클로즈업 샷 (16:9) |
| **피사체** | 모던한 오픈 키친 중앙에서 세련된 착장의 여성 요리연구가가 로컬 식재료를 섬세하게 디쉬에 플레이팅하는 모습 |
| **배경** | 예당호 뷰가 은은하게 비치는 정갈하고 모던한 미식 공간 (은은한 핀 조명) |
| **텍스트 유무** | 있음 - 핵심 슬로건 자막 |

### 내레이션 및 음향
| 항목 | 내용 |
|------|------|
| **내레이션** | 예당호의 신선함이 그대로 담겨 있습니다. |
| **화면 카피** | "예당의 맛" (자막, 1.5초 노출) |
| **배경음악/음향** | 섬세한 플레이팅 소리 (집게 소리, 정갈한 오케스트라 첼로/피아노 선율) |

### 제작 도구 및 방법

#### 📸 이미지 생성 도구
| 항목 | 내용 |
|------|------|
| **도구명** | Gemini Flash |
| **사용 목적** | 여성 요리연구가의 고급스러운 플레이팅 오프닝 장면 생성 |
| **입력 프롬프트** | `A stylish female chef delicately plating a fine-dining dish using fresh local ingredients, modern open kitchen with a subtle view of Yedang Lake, warm elegant spot lighting, ultra-detailed, cinematic 16:9` |
| **출력 결과 요약** | 요리연구가의 정갈하고 품격 있는 파인다이닝 플레이팅 장면 |
| **생성 파일명** | `Scene2_Chef_Plating_v1.png` |

#### 🎥 비디오 생성/변환 도구
| 항목 | 내용 |
|------|------|
| **도구명** | Flow |
| **사용 목적** | 플레이팅 손짓 및 음식을 완성하는 동적 커트 구성 |
| **입력 프롬프트** | `Slow-motion video of an elegant chef's hands finishing a fine-dining plate, subtle steam rising, warm luxurious ambience, cinematic depth of field, 2.5 seconds` |
| **출력 결과 요약** | 2.5초의 감각적이고 고급스러운 조리/플레이팅 영상 |
| **생성 파일명** | `Scene3_FineDining_Video_2.5s_v1.mp4` |

#### 🎵 오디오 생성 도구
| 항목 | 내용 |
|------|------|
| **도구명** | ElevenLabs (AI 음성) |
| **사용 목적** | 정갈하고 고급스러운 분위기의 클래식/앰비언트 사운드 효과 |
| **입력 프롬프트** | `Gentle clinking of porcelain, subtle kitchen ambient sound, elegant cello melody, premium atmosphere, 2.5 seconds` |
| **출력 결과 요약** | 고급 파인다이닝에 어울리는 감성적인 오디오 트랙 |
| **생성 파일명** | `Scene3_Audio_FineDining_v1.mp3` |

### 최종 산출물
| 항목 | 내용 |
|------|------|
| **최종 영상 파일** | `Scene3_Complete_3s.mp4` |
| **해상도** | 1920x1080 (Full HD) |
| **프레임레이트** | 30fps |
| **비트레이트** | 5000kbps |

---

## 🎬 씬 3: 엔딩 - 프리미엄 미식 경험 & 브랜딩

### 기본 정보
| 항목 | 내용 |
|------|------|
| **씬 번호** | 3 |
| **씬 길이** | 3초 |
| **목표 메시지** | 브랜드 로고와 함께 '예당 스토리 테이블'만의 특별한 미식 여정 방문을 유도한다 |

### 화면 구성
| 항목 | 내용 |
|------|------|
| **구도** | 풀 샷 -> 로고 플레이스먼트 (16:9) |
| **피사체** | 통창 너머 예당호가 펼쳐진 럭셔리한 테이블, 미소를 지으며 다이닝을 즐기는 방문객들, 로고 그래픽 |
| **배경** | 예당호의 노을/풍경이 조화롭게 어우러진 모던 다이닝 홀 |
| **텍스트 유무** | 있음 - 브랜드 로고, 대표 슬로건 및 예약/방문 CTA |

### 내레이션 및 음향
| 항목 | 내용 |
|------|------|
| **내레이션** | "맛있게 드시고 좋은 추억 만드세요" (브랜드명 명확하고 신뢰감 있는 톤) |
| **화면 카피** | **예당 스토리 테이블** (브랜드 로고 중심) |
| **배경음악/음향** | 음악 페이드아웃 + 해돋이/노을 분위기의 클라이맥스 BGM |

### 제작 도구 및 방법

#### 📸 이미지 생성 도구
| 항목 | 내용 |
|------|------|
| **도구명** | Gemini Flash |
| **사용 목적** | 통창 너머 호수를 배경으로 펼쳐지는 미식 테이블 및 브랜드 분위기 완성 |
| **입력 프롬프트** | `A high-end restaurant dining table overlooking Yedang Lake during dusk, elegant table setting with artistic dishes, warm ambient lighting, peaceful premium atmosphere, 16:9 cinematic shot` |
| **출력 결과 요약** | 예당호 뷰를 담은 프리미엄 미식 공간의 대표 비주얼 |
| **생성 파일명** | `Scene4_Dining_Space_Main_v1.png` |

#### 🎥 비디오 생성/변환 도구
| 항목 | 내용 |
|------|------|
| **도구명** | Flow |
| **사용 목적** | 호수 뷰 카메라 아웃 모션 및 모션 그래픽 브랜드 로고 페이드인 |
| **입력 프롬프트** | `Slow pull-back camera motion showing the luxury table and Yedang Lake view, ending with elegant logo transition and fade out, 3 seconds` |
| **출력 결과 요약** | 여운을 남기는 호수 뷰 엔딩 및 브랜드 타이틀 모션 영상 |
| **생성 파일명** | `Scene3_Ending_Video_3s_v2.mp4` |

#### 🎵 오디오 생성 도구
| 항목 | 내용 |
|------|------|
| **도구명** | ElevenLabs (Voice) |
| **사용 목적** | 브랜드 성우 내레이션 및 엔딩 BGM 완성 |
| **입력 프롬프트** | `Sophisticated Korean male or female voice saying "Yedang Story Table", luxury brand tone, accompanied by gentle musical resolving chord` |
| **출력 결과 요약** | 신뢰감 있고 품격 있는 브랜드 엔딩 나레이션 음원 |
| **생성 파일명** | `Scene3_Audio_BrandJingle_v1.mp3` |

### 최종 산출물
| 항목 | 내용 |
|------|------|
| **최종 영상 파일** | `Scene3_Complete_3s.mp4` |
| **해상도** | 1920x1080 (Full HD) |
| **프레임레이트** | 30fps |
| **비트레이트** | 5000kbps |

https://github.com/user-attachments/assets/24e928a9-3dbb-410e-88af-d022cd1a816a


## STEP 8 · 최소 1개 씬에서 프롬프트 수정 전/후 기록하기

씬3의 프롬프트 수정함

수정 전: 씬3의 마지막 부분의 콘텐츠 명과 회사명이 잘못 보여짐

수정 후: 기존 프롬프트에 "기존 파라미터(구도, 색감, 캐릭터,배경 등) 유지하고 마지막 로고 내용을 수정해줘.

수정내용은 "예당 스토리 테이블" 줄바꿔서 "온기로"" 로 재 요청함

결과: 콘텐츠명과 회사명으로 수정됨

## STEP 9 · 생성형 AI 도구 3종 이상 준비하기

이미지 생성 AI: Gemini Flash

비디오 생성 또는 이미지→비디오 변환 AI: Flow

오디오 생성 AI Suno(음악), ElevenLabs(음성) 

## STEP 10 · 대체 도구도 준비하기

이미지 생성 대체 도구: 
GPT 무료버전
비디오 생성/변환 대체 도구:
kling
오디오 생성 대체 도구: 
Suno, Flow

## STEP 11 · 이미지 생성 단계 먼저 진행하기

순서:

씬별로 필요한 이미지 스타일을 정한다.
이미지 생성 프롬프트를 작성한다.
브랜드 톤에 맞는 결과가 나오는지 확인한다.
스토리보드를 이미지 기준으로 먼저 확정한다.
이 단계에서

구도
색감
캐릭터
배경
텍스트 공간
등을 맞춰야 이후 영상화가 쉬워짐

## STEP 12 · 스타일/캐릭터 일관성 유지 전략 정하기

같은 색감 유지
같은 캐릭터/의상/세계관 유지
같은 화면 비율 유지
가능하면 스타일 레퍼런스 기능 활용 여부 기록
Character Reference
Style Reference
기타 제어 기능

## STEP 13 · 비디오 생성 또는 변환 진행하기

이미지 생성 결과를 바탕으로

짧은 모션 영상으로 만들거나
텍스트/이미지 기반 영상 생성
을 진행

어떤 이미지/프롬프트를 영상화했는지
어떤 움직임을 의도했는지
씬 길이에 맞게 결과가 나왔는지 체크함

## STEP 14 · 오디오 요소 생성하기

배경음악
효과음
AI 음성 내레이션을 포함함

어떤 오디오를 넣을지
브랜드 분위기와 맞는지
어떤 도구로 만들었는지
어떤 장면에 붙일지 체크함

## STEP 15 · 최종 영상의 스토리라인 점검하기

기승전결 구조
문제 → 전환 → 해결
명확한 가치 전달 구조
누구에게 어떤 가치인지 분명하게 전달할 것(광고성)을 목표로 제작함

## STEP 16 · 마지막 3~5초 브랜드 노출 넣기

로고
브랜드명
슬로건
CTA(Call to Action) 이부분은 사용안함

## STEP 17 · 파일명 규칙 정하기

씬 번호 방식
이미지/영상/오디오 구분 방식
버전 표기 방식
최종 파일명 방식
예:

scene01_img_v1.png
scene01_vid_v1.mp4
scene01_bgm_v1.wav
final_brand_ad.mp4

## STEP 18 · 크레딧/비용 부족 시 대응 전략 정하기

씬 수 줄이기
정지 이미지 + 간단한 모션 중심으로 구성하기
같은 스타일 레퍼런스를 고정해 재생성 줄이기
이 전략을 실제 제작에 반영함

## STEP 19 · 보너스 과제

9:16 화면 비율 버전

https://github.com/user-attachments/assets/2ae2a6e3-5253-4d2c-9c43-1f4c03fc25db
